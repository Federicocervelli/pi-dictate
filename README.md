...
The model is cached under `~/.cache/moonshine_voice/`. The server listens only on localhost and uses no API key or remote inference. In Pi, `/tts on`, `/tts off`, and `/tts status` control the service.
